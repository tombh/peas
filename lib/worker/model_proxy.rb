# This class allows for the cosmetic convenience of being able to chain
# a method-call onto `worker()` like so; `self.worker(:optimal_pod).scale(web: 1)`
module Peas
  module ModelWorker
    class ModelProxy
      def initialize(pod, block_until_complete, instance)
        raise PeasError, "Trying to create a worker job without specifying a pod to run it on." unless pod
        @pod = pod
      ...
      def worker_manager(method, *args, &block)
        ...
        if block_given? || @block_until_complete
          ...
          handle_status method, status, progress, &block
        end
        new_job_id
      end
